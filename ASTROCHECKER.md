...
Avvia `AstroChecker.exe` dalla cartella dell'applicazione: il browser apre una porta libera di localhost. Il catalogo SQLite è già incluso e viene controllato automaticamente. SkyChart non è più un prerequisito.

Per sviluppo dal sorgente:
...
Gli esempi di accettazione sono in `tests/test_acceptance.py` e `tests/test_local_acceptance.py`.
...
Il progetto esegue i calcoli localmente e non richiede SkyChart o una
connessione durante l’uso.
